# DexJoCo 시뮬레이션에서 offline RL 파이프라인 재현

목적: real 에서 검증이 너무 비싼 루프 (BC → 롤아웃 → critic → test-time guidance) 를
**성공률을 싸게 셀 수 있는 sim** 에서 한 번 돌려본다. BC 이후 단계는 `rl/` 코드를
그대로 재사용하고, sim 전용으로 새로 필요한 것은 **데이터 컨버터** 와 **롤아웃
클라이언트** 둘뿐이다.

벤치마크: `third_party/dexjoco` — DexJoCo (arXiv 2605.16257), MuJoCo 기반 dexterous
manipulation 11 태스크. Franka Panda + Allegro hand.

> **결정됨 (2026-08-27): §4.3 의 선택지 C — 지금의 EEF action 그대로 파이프라인을 먼저
> 관통시킨다.** joint 공간 전환(B) 은 critic 이 EEF state/action 에서 학습이 안 되거나
> openarm 과의 차이가 원인으로 의심될 때 되돌아온다.
>
> 아래는 원래의 결정 문맥 — **"openarm 처럼 joint pose action 으로 갈 것인가"**:
> dexjoco 의 팔은 토크 actuator + opspace 라서 joint 목표를 직접 넣는 경로가 없고, 팔 joint
> 각도가 raw 데이터에도 없다. 세 선택지(A/B/C) 와, dexjoco 를 안 쓰는 대안(GR1 tabletop —
> 이미 joint 공간이고 롤아웃 하네스·데이터셋이 다 있다) 을 §4.3 에 정리했다.
> 나머지 항목은 벤치마크가 정한 사실이거나 코드가 강제하는 제약이라 선택의 여지가 없다.
